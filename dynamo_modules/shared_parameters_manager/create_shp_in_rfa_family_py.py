...
class PyRevit(Singleton):
    ...
    class CombinedDefinitionUtils():
        ...
        spec_type_id_to_txt_dict = {
            ...
            # SpecTypeId.Boolean.YesNo: {"type": "Yes/No", "discipline": "Common"},
        ...
        revit_pg_names_to_rvt_pg_dict = {
            ...
            # "Other": BuiltInParameterGroup.PG_OTHER,
...
# Add XLS definitions to external Definitions
if write_to_txt:
    report_1 = PyRevit.CombinedDefinitionUtils.create_new_external_definitions()
# if write_to_rvt:
#     report_2 = PyRevit.CombinedDefinitionUtils.create_new_internal_definitions()
# if write_to_excel:
#     PyRevit.CombinedDefinitionUtils.export_data_to_exvcel()

families_doc_lst = open_family_documents()
#
ordered_shared_parameters_names_lst = []
for combined_definition in PyRevit.CombinedDefinitionPy.instances:
    ordered_shared_parameters_names_lst.append(combined_definition.name)
#
outcomes = []
# Collect values
for rvt_family in families_doc_lst:
    if rvt_family.IsFamilyDocument:
        ...
        for combined_definition in PyRevit.CombinedDefinitionPy.instances:
            ...
            b = PyRevit.CombinedDefinitionUtils.revit_pg_names_to_rvt_pg_dict.get(combined_definition.xls_imp_definition.internal_group)
            i = True if str(combined_definition.xls_imp_definition.binding_type) == "1" else False
            error_temp.append((combined_definition.name, d, b, (combined_definition.xls_imp_definition.internal_group), i))
            # outcomes.append((d, b, i))
            try:
                TransactionManager.Instance.EnsureInTransaction(rvt_family)
                new = rvt_family.FamilyManager.AddParameter(d, b, i)
        ...
        for param_def in family_parameters_list:
            ...
            # parameter_unit_type = param_def.GetUnitTypeId.TypeId # As ForgeTypeId
            # pg_group = ParameterUtils.GetBuiltInParameterGroup()
            error_temp.append((param_def_name))
            combined_family_parameters_list.append((param_def, param_def_name))
        combined_family_parameters_list = ListUtils.sort_sublist_by_pattern(ordered_shared_parameters_names_lst,
...
families_doc_lst = [families_doc_lst, outcomes, error_temp]
