...
class PyRevit(Singleton):
    ...
    class CombinedDefinitionUtils():
        ...
        def export_data_to_exvcel(cls):
            ...
            writer.write_data(worksheet_name="FAMILY CURRENT SHP", data=xls_export_data, start_row=2, start_col=1)

        @classmethod
        def export_data_to_excel(cls, xls_export_data):
            # Write data to excel
            writer = WriteXLS(file_path=output_excel_path_string)
            # Write data to the specified worksheet starting at row 2, column 1
            writer.write_data(worksheet_name="FAMILY CURRENT SHP", data=xls_export_data, start_row=2, start_col=1)
...
for rvt_family in families_doc_lst:
    if rvt_family.IsFamilyDocument:
        ...
        for param_def in family_parameters_list:
            if True:
            # if param_def.IsShared:
                param_def_name = param_def.Definition.Name
                param_def_pg_group = PyRevit.CombinedDefinitionUtils.revit_pg_names_to_string_dict.get(str(param_def.Definition.ParameterGroup), " ")
                parameter_unit_type = str(param_def.StorageType) # As ForgeTypeId
                param_instance = "1" if param_def.IsInstance else "0"
                outcome.append([param_def_name, param_def_pg_group, param_instance , parameter_unit_type])
        output = []
        for i,row in enumerate(outcome,1):
            row = [str(i)] + row
            output.append(row)
...
families_doc_lst = outcomes[0]
PyRevit.CombinedDefinitionUtils.export_data_to_excel(families_doc_lst)
